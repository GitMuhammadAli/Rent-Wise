            ...
            {comments.length > 0 ? (
                comments.map(comment => (
                    <Box key={comment._id} borderWidth={1} borderRadius="md" p={4}>
                        <HStack>
                            <Avatar src={`${import.meta.env.VITE_BACK_END_URL}${comment.author.imageUrl}`} name={comment.author.name} size="sm" />                            <Text fontWeight="bold">{comment.author.name}</Text>
                            ...
                            <VStack spacing={2} mt={4} pl={4} align="stretch">
                                {comment.replies.map(reply => (
                                    <Box key={reply._id} borderLeftWidth={1} pl={2}>
                                        <HStack>
                                            <Avatar src={reply.author?.imageUrl} name={reply.author?.name} size="xs" />
